		 ...
		 bam_fetch_f func, int flagFilter, int alignmentFlag, int strandFlag)
{
	...
	// If neither flag is set we can simply use the original function:
	if(!alignmentFlag && !flagFilter && !strandFlag){
		while ((ret = bam_iter_read(fp, iter, b)) >= 0) func(b, data);
	}else{
		// we have to check the flags for each section!
		while ((ret = bam_iter_read(fp, iter, b)) >= 0){
			// if alignmentFlag is set check this first:
			if(flagFilter & b->core.flag)
				continue;
			// if the alignmentFlag is set we only check whether the
			// 1st bit is set. If the first bit is set we only accept
			// forward matches. This is checked by shifting the bit
			// 4 positions and performing a binary AND against the alignment
			// flag (0x16, or the 4th bit indicates a reverse strand).
			// For any higher bits we only accept reverse strands.
			if(strandFlag && ( (strandFlag << 4) & b->core.flag ))
				continue;
			// if none of the first third flags are set we can just call the
			// function and continue
			if(!(alignmentFlag & 7)){
				func(b, data);
				continue;
			}
			// otherwise we have to look at the positions of the alignment
			int32_t b_beg = b->core.pos;
			if( (alignmentFlag & 1) && b_beg < beg )
				continue;

			int32_t b_end = b_beg;
			// we need to find the end position either if the second bit is set
			// or if the 3rd bit is set but the begin position is outside of the range.
			if(alignmentFlag & 2 || ( b_beg < beg && alignmentFlag & 4 )){ 
			    uint32_t i;
			    for(i = 0; i < b->core.n_cigar; ++i)
				    b_end += b->cigar[i] >> BAM_CIGAR_SHIFT;
			}
			// if the 2nd bit is set, the end has to be within the range
			if((alignmentFlag & 2) && (b_end > end))
				continue;
			// if the 3rd bit is set, then either is ok, so we can say
			if((alignmentFlag & 4) && (b_beg < beg && b_end > end))
				continue;
			// if we get here we just call the function:
			func(b, data);
		}
	}
...
// calls bam_fetch_2f with the two flag filters set to 0.
int bam_fetch(bamFile fp, const bam_index_t *idx, int tid, int beg, int end, void *data, bam_fetch_f func)
{
	bam_fetch_2f(fp, idx, tid, beg, end, data, func, 0, 0, 0);
}
